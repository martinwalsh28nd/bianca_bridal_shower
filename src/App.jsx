...
const STORAGE_KEY = "bianca-shower-rsvps";
...
export default function App() {
  ...
  const submitRsvp = async () => {
    ...
    const entry = { id: Date.now(), ...form, at: new Date().toISOString() };
    const next = [...rsvps, entry];
    setRsvps(next);
    persist(next);
    setSubmitted(true);
    // Fire-and-forget notification — don't block the confirmation screen
    try {
      ...
      // Notification failure is silent — the RSVP is still saved
        ...
        {tab === "rsvp" && (
          submitted ? (
            <div style={{ ...cardStyle, textAlign: "center", padding: "3.5rem 2rem" }}>
              ...
              <button onClick={() => { setSubmitted(false); setForm({ firstName: "", lastName: "", email: "", phone: "", attendance: "yes", dietary: "", note: "" }); }} style={outlineBtn}>
            ...
            <div style={cardStyle}>
              ...
              <button onClick={submitRsvp} disabled={!formValid}
                style={{ ...primaryBtn, width: "100%", opacity: formValid ? 1 : 0.45, padding: "13px" }}>
                Submit RSVP
              </button>
            </div>
          )
